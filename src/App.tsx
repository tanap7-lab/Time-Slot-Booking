...
export default function App() {
  ...
  const handleAdminCancel = async (per_no: string, slot_id: number) => {
    try {
      const res = await fetch("/api/bookings/cancel", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteEmployee = async (per_no: string) => {
    if (!window.confirm("Delete this personnel record permanently?")) return;
    try {
  ...
  const handleDeleteSlot = async (slot_id: number) => {
    if (!window.confirm("Delete this time slot? This will cancel all associated bookings.")) return;
    try {
        ...
        <main className="max-w-4xl mx-auto p-10">
              ...
              {userBooking ? (
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <button 
                      onClick={() => {
                        if (window.confirm("Are you sure you want to cancel your current booking? This action cannot be undone.")) {
          ...
          <div className="space-y-6">
            <h3 className="text-2xl font-bold flex items-center gap-3">
               Available Windows <span className="text-brand-primary font-bold bg-brand-bg px-3 py-1 rounded-full text-sm">{currentUser.assigned_date}</span>
            </h3>
               ...
               {availableSlots.length === 0 ? (
                 ...
                 availableSlots.map(slot => {
                   ...
                   return (
                     <div 
                      key={slot.slot_id}
                      className={`bg-white p-8 rounded-2xl border-2 transition-all flex items-center justify-between ${
                        isBookedByMe ? "border-brand-primary shadow-md" : "border-slate-100"
                        ...
                        {isBookedByMe ? (
                          ...
                          <button 
                            ...
                            className={`px-8 py-3 rounded-xl text-xs font-bold transition-all ${
                              isFull ? "bg-slate-100 text-brand-muted cursor-not-allowed" : 
                      ...
                      <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
                        <button
                          ...
                          className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all ${
                            importDateFormat === "UK" ? "bg-white text-brand-primary shadow-sm" : "text-slate-400 hover:text-slate-600"
                          }`}
                        >
                          UK (DD/MM)
                        </button>
                        <button
                          ...
                          className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all ${
                            importDateFormat === "US" ? "bg-white text-brand-primary shadow-sm" : "text-slate-400 hover:text-slate-600"
                          }`}
                        >
                          US (MM/DD)
                        </button>
                  ...
                  <div className="space-y-4">
                    {slots.map((slot) => (
                      <div 
                        ...
                        className={`bg-white p-6 rounded-2xl border-2 transition-all cursor-pointer group flex items-center justify-between ${
                          selectedSlot === slot.slot_id ? "border-brand-primary shadow-md" : "border-slate-100 hover:border-slate-200"
                        }`}
                      >
                        <div className="flex items-center gap-6">
                           <div className={`w-14 h-14 rounded-xl flex items-center justify-center transition-colors ${
                             selectedSlot === slot.slot_id ? "bg-brand-primary text-white" : "bg-brand-bg text-brand-muted"
                           }`}>
                             <Clock className="w-8 h-8" />
                           </div>
                           <div>
                         ...
                         <div className="flex items-center gap-2">
                           ...
                           <ChevronRight className={`w-5 h-5 transition-transform duration-300 ${
                             selectedSlot === slot.slot_id ? "translate-x-1 text-brand-primary" : "text-slate-300 group-hover:translate-x-1"
                           }`} />
                         </div>
                      </div>
                    ))}
                             ...
                             bookings.filter(b => b.slot_id === selectedSlot).map((b, idx) => (
                               <div key={idx} className="flex items-center justify-between p-4 bg-slate-50/50 rounded-xl border border-slate-50 transition-all hover:bg-slate-50 group/item">
                                ...
                                <button 
                                  ...
                                  className="p-2 text-slate-300 hover:text-red-500 hover:bg-white rounded-lg transition-all relative z-10 border border-transparent hover:border-red-50 active:scale-95"
                          ...
                          <div className="flex items-center gap-3 text-brand-muted">
                            ...
                            <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-emerald-500 border border-slate-100 shadow-sm mr-2">
                             ...
                             <button 
                               ...
                               className="w-12 h-12 bg-red-50 rounded-full flex items-center justify-center text-red-600 border border-red-100 shadow-sm hover:bg-red-600 hover:text-white transition-all opacity-0 group-hover:opacity-100 relative z-10 active:scale-95"
